...
- 상태: RESOLVED
- 심각도: LOW
- 발견 BE: BE-0013 typecheck
- 관련 contract/migration revision: `FAE2` ciphertext envelope / DB migration 없음
- 내용: ciphertext 마지막 byte를 직접 XOR한 test가 `noUncheckedIndexedAccess`에서 possibly undefined로 판정돼 첫 platform typecheck가 실패했다.
- 영향: production crypto 구현에는 영향이 없고 tamper test compile만 실패했다.
- 해결 조건: bounded fallback으로 byte를 읽고 명시적으로 다시 써 strict typecheck와 crypto test를 통과할 것.
- 목표 BE: BE-0013
- 해결 BE: BE-0013
- 검증: platform strict typecheck와 crypto/MyData/OIDC 관련 19 tests, Testcontainers 8 tests를 통과했다.

### BE-ISSUE-0005 — Standalone smoke의 stale host build

- 상태: RESOLVED
- 심각도: MEDIUM
- 발견 BE: BE-0012 local Compose smoke
- 관련 contract/migration revision: `0007_finapp_outbox`
- 내용: `make infra-up`은 Docker image만 build하므로 host `dist`가 이전 commit이면 `make smoke-test`의 별도 platform process가 오래된 코드를 실행했다.
- 영향: migration과 container는 정상이어도 새 outbox worker가 없는 host process가 실행되어 smoke가 timeout됐다.
- 해결 조건: standalone smoke가 필요한 host build를 선행하고 actual flow가 outbox processed/delivery를 확인할 것.
- 목표 BE: BE-0012
- 해결 BE: BE-0012
- 검증: `make smoke-test`가 platform build를 선행한 뒤 actual OIDC/business flow와 outbox 3/3을 통과했다.

### BE-ISSUE-0004 — 존재하지 않는 PostgreSQL JSONB length 함수 사용

- 상태: RESOLVED
- 심각도: LOW
- 발견 BE: BE-0012 Testcontainers
- 관련 contract/migration revision: `0007_finapp_outbox`
- 내용: redacted payload test가 PostgreSQL에 없는 `jsonb_object_length`를 사용해 첫 migration integration run이 실패했다.
- 영향: production code/migration 영향은 없고 검증 query만 실패했다.
- 해결 조건: 지원되는 JSONB 연산으로 exact allowlist key 검사를 수행할 것.
- 목표 BE: BE-0012
- 해결 BE: BE-0012
- 검증: 허용 key를 차감한 JSONB가 빈 object인지 검사하도록 바꾸고 Testcontainers 8 tests를 통과했다.
