...
- platform Drizzle history는 `finapp_meta.finapp_platform_drizzle_migrations`, simulator Drizzle history는 `finapp_meta.finapp_simulator_drizzle_migrations`를 사용한다.
...
- shared/demo/production DB에서 `drizzle-kit push`와 destructive reset을 금지하고 versioned migration만 적용한다.
...
- Testcontainers for Node.js에서 role isolation test를 추가해야 한다.
