...
`내 정보` 탭 시연 시에는 기준 화면과 같은 overview가 상단·하단 앱 바 없이 열리는 것을
보여주고, `내 정보 관리`를 눌러 기존 계정·투자 성향 관리 화면으로 이동한다. overview의
자동로그인/생체인증 switch는 포트폴리오 화면 상태만 바뀌며 실제 설정 저장은 아직 하지
않는다.
