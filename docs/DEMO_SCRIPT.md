...
`내 정보` 탭 시연 시에는 다른 메인 탭과 동일하게 공통 상단바의 WM 로고·알림함과 하단
내비게이션이 함께 표시되는 overview를 보여준다. `내 정보 관리`를 누르면 공통 상·하단
바가 없는 full-screen 관리 화면으로 이동한다. overview의 자동로그인/생체인증 switch는
포트폴리오 화면 상태만 바뀌며 실제 설정 저장은 아직 하지 않는다.

overview의 `알림 설정`을 누르면 별도 full-screen 알림 설정 화면으로 이동한다. 서비스 이용
알림은 ON, 나머지 알림은 OFF로 시작하며 switch를 눌러 화면 상태가 바뀌는 것과 상단 뒤로가기
동작을 보여준다. 마케팅 정보 활용 동의 버튼도 포트폴리오용 local visual state만 변경한다.
